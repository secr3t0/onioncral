...
DEFAULT_RETRIES = 5
DEFAULT_TIMEOUT = 60
CHUNK_SIZE = 8 * 1024  # 8 KB streaming read granularity. Kept small
                        # deliberately: Tor circuits can be extremely
                        # slow (single-digit KB/s is common), and too
                        # large a chunk makes a genuinely slow-but
                        # -still-flowing download indistinguishable
                        # from one that has actually stalled, since a
                        # single chunk read blocks until that many
                        # bytes have arrived.
...
class TorSession:
    ...
    def __init__(self, proxy: str, timeout: int, logger: logging.Logger,
        ...
        # Separate, much more generous allowance for a single chunk
        # read mid-download: Tor circuits can be genuinely very slow
        # (a few KB/s is common), so this needs to tolerate real but
        # slow progress and only fire when a read has truly stopped.
        self._stall_timeout = stall_timeout

    def _run_with_watchdog(self, func, watchdog_timeout: float, *args, **kwargs):
        """Run func(*args, **kwargs) on a throwaway daemon thread and
        wait at most watchdog_timeout seconds for it. Daemon threads
    ...
    def get(self, url: str, stream: bool = False) -> requests.Response:
        try:
            return self._run_with_watchdog(
                self.session.get, self._hard_timeout, url, timeout=self.timeout, stream=stream
            )
    ...
    def iter_content_with_watchdog(self, resp: requests.Response, chunk_size: int):
        ...
        subject to a hard wall-clock watchdog (self._stall_timeout,
        deliberately much more generous than the connect-phase
        watchdog since Tor circuits can be genuinely very slow).
        Without this, a stall mid-download (after headers are already
        received) could still block a worker forever even though the
        connect/first-byte phase is protected."""
        iterator = resp.iter_content(chunk_size=chunk_size)
        sentinel = object()
        while True:
            try:
                chunk = self._run_with_watchdog(next, self._stall_timeout, iterator, sentinel)
            except requests.exceptions.Timeout:
                self.logger.warning(
                    f"Stall watchdog timeout ({self._stall_timeout}s) exceeded mid-download "
                    f"with no bytes received in that window; the underlying socket appears "
                    f"stuck. Abandoning this attempt."
                )
                raise
                ...
                with open(tmp_path, "wb") as f:
                    for chunk in self.session.iter_content_with_watchdog(resp, CHUNK_SIZE):
                        ...
                        if now - last_heartbeat >= 10:
                            self.logger.info(
                                f"  ... still receiving {url}: {downloaded} bytes so far"
                            )
                            last_heartbeat = now
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--timeout", type=int, default=DEFAULT_TIMEOUT,
                    help=f"Per-request connect timeout in seconds (default: {DEFAULT_TIMEOUT}).")
    p.add_argument("--stall-timeout", type=int, default=300,
                    help="Max seconds a single chunk-read may take mid-download before it's "
                         "treated as stuck and retried (default: 300). Raise this on very "
                         "slow Tor circuits; lower it to detect stalls faster.")
...
def main() -> None:
    ...
    session = TorSession(args.proxy, args.timeout, logger, stall_timeout=args.stall_timeout)
